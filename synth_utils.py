...
def unrotate2d_(pts):
    ...
    if np.any(~np.isfinite(pts)):
        print("Warning: Invalid points detected in unrotate2d")
        # 移除无效点
        pts = pts[np.all(np.isfinite(pts), axis=1)]
        
    if pts.shape[0] < 2:
        print("Warning: Not enough valid points for rotation calculation")
        return np.eye(2)  # 返回单位矩阵（不旋转）
    ...
    # 添加数值稳定性检查
    if np.all(np.abs(pts) < 1e-10):
        print("Warning: Points too close to origin")
        return np.eye(2)
    ...
    try:
        ...
        print(f"Warning: Error in rotation calculation: {e}")
        return np.eye(2)  # 返回单位矩阵作为后备方案