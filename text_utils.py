...
class RenderFont(object):
    ...
    def render_curved_(self, font, text_a, text_b):
        ...
        if not isword or wl > 10 or wl == 1 or wl_ == 1 or np.random.rand() > self.p_curved:
        ...
        rots_ = [-int(math.degrees(math.atan(BS['diff'](i-mid_idx_)/(font.size/2)))) for i in range(wl_)]
        ...
        ch_bounds_ = font.render_to(surf_, rect_, text_b[mid_idx_], rotation=rots_[mid_idx_])
        ...
        for i in range(wl_):
            ...
            newrect_.centery = max(newrect_.height, min(fsize[1] - newrect_.height, newrect_.centery + curve_[i]))
            try:
        ...
        bbs_ = np.array(bbs_)
        surf_arr_, bbs_ = crop_safe(pygame.surfarray.pixels_alpha(surf_), rect_union_, bbs_, pad=5)
    ...
    def render_sample(self,font,mask):
        ...
        while i < self.max_shrink_trials and max_font_h > self.min_font_h:
            ...
            # render the text:
            txt_arr,txt,bb, txt_arr_,txt_,bb_ = self.render_curved_(font, text_a, text_b)
            ...
            if np.any(np.r_[txt_arr.shape[:2]] > np.r_[mask.shape[:2]]) or np.any(np.r_[txt_arr_.shape[:2]] > np.r_[mask.shape[:2]]):
            ...
            if len(loc) > 0:
                a_center = loc[0] + np.array([txt_arr.shape[0]//2, txt_arr.shape[1]//2])
                b_offset = a_center - np.array([txt_arr_.shape[0]//2, txt_arr_.shape[1]//2])
                ...
                if (y >= 0 and x >= 0 and 
                    ...
                    if (len(bb.shape) == 3 and bb.shape[0] == 2 and bb.shape[1] == 4 and
                        len(bb_.shape) == 3 and bb_.shape[0] == 2 and bb_.shape[1] == 4):
...
class FontState(object):
    ...
    def get_aspect_ratio(self, font, size=None):
        ...
        try:
            ...
            r[non_zero_indices] = np.abs(sizes[non_zero_indices, 1] / sizes[non_zero_indices, 0])  # 仅计算非零部分
