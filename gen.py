...
def save_image_and_labels(imname, res, output_dir):
    ...
    for i, result in enumerate(res):
        ...
        labels = {
            ...
            "lineBB": result["lineBB"].tolist(),
            "line_texts": result["line_texts"],
            "text_index": result["text_index"],
        }
        labels_ = {
            "charBB": result["charBB_"].tolist(),
            # "wordBB": result["wordBB_"].tolist(),
            "txt": [text for text in result["txt_"]],
            "lineBB": result["lineBB_"].tolist(),
            "line_texts": result["line_texts_"],
            "text_index": result["text_index_"],
        }
        from synthgen import save_visualization

        # save_visualization(imname, res, output_dir)
        # 保存标签为 JSON 文件
...
def main(viz=False, db=False):
    ...
    if db:
        ...
        print(
            colorize(
                Color.GREEN, "Storing the output in: " + config["out_dir"], bold=True
            )
        )
    ...
    for i in range(start_idx, end_idx):
        ...
        try:
            ...
            res = RV3.render_text(
                img, depth, seg, area, label, ninstance=INSTANCE_PER_IMAGE, viz=viz
            )
            if len(res) > 0:
                if not db:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="db",
        default=False,
        help="flag to not store results in the database",
    )
